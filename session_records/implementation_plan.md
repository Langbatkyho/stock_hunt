...
## Phase 3: Cải tiến RSI Range Shift

**Mục tiêu:** Cung cấp mảng 30 giá trị RSI (14) gần nhất cho AI phân tích, giúp AI nhận diện xu hướng động (trend) thay vì chỉ một điểm số tĩnh.

### 1. Thay đổi Data Contract (`stock_hunt/session_records/data_contract.md`)
- Bổ sung khóa mới: `rsi_14_history` (kiểu `List[float]`, 30 phần tử).
- Giữ nguyên khóa `rsi_14` hiện tại để duy trì tính tương thích ngược với bảng hiển thị của Telegram Bot.

### 2. Cập nhật Engine tính toán (`bot_app/indicators.py`)
- Trong hàm `calc_ta_indicators` (hoặc `build_scorecard`), trích xuất mảng 30 giá trị RSI gần nhất từ `ta.momentum.rsi(length=14)` (loại bỏ giá trị NaN).
- Lưu danh sách này vào dict kết quả dưới dạng `rsi_14_history`.

### 3. Cập nhật AI Analyzer (`stock_hunt/ai_analyzer.py`)
- Khi xây dựng Prompt (`generate_prompt`), nếu tồn tại `rsi_14_history`, thực hiện tính toán:
  - Giá trị lớn nhất (Max), nhỏ nhất (Min) và độ lệch chuẩn (Std Dev) trong 30 phiên.
  - Chuyển đổi mảng thành định dạng chuỗi thời gian: `[T-29: 45.1, T-28: 46.2, ..., T-0: 50.1]`.
- Nhúng các thông tin này vào Prompt tiếng Việt để tối ưu hoá khả năng đọc hiểu chuỗi thời gian của mô hình Gemini Flash 3.5.

### 4. Cập nhật Scanner (`stock_hunt/scanner.py`)
- Đảm bảo thuộc tính `rsi_14_history` đi qua bộ chuyển đổi native Python serialization (không bị kẹt ở định dạng mảng numpy/pandas).
